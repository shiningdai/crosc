...
func opBload(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	v := scope.Stack.peek()
	mIndex := int64(v.Uint64())
	v.SetBytes(scope.Buffer.GetPtr(mIndex))
	return nil, nil
}

func opBstore(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	// pop value of the stack
	mIndex := scope.Stack.pop()
	val := scope.Stack.pop()
	scope.Buffer.Set32(mIndex.Uint64(), &val)
	return nil, nil
}
