...
ASTPointer<ContractDefinition> Parser::parseContractDefinition()
{
	...
	// vector<ASTPointer<VariableDeclaration>> stateVarDeclNodes;
	...
	try
	{
		...
		while (true)
		{
			...
			else if (variableDeclarationStart())
			{
				...
				// options.kind = VarDeclKind::State;
				// options.allowInitialValue = true;
				// string tokenLiteral = m_scanner->currentLiteral();
				// size_t strLen = tokenLiteral.length();
				// size_t bitLen = 0;
				// char num;
				// for (size_t i = 0; i < 3; i++)
				// {
				// 	num = tokenLiteral[strLen - 3 + i];
				// 	bitLen = bitLen * 10 + (size_t)(num - '0');
				// } 
				// bits.push_back(bitLen);
				// stateVarDeclNodes.push_back(parseVariableDeclaration(options));
				// expectToken(Token::Semicolon);
			}
	...
	if (m_inParserRecovery)
		...
		expectToken(Token::RBrace);
	// // handle state variable declarition nodes
	// vector<ASTPointer<VariableDeclaration>> varBit128;
	// vector<ASTPointer<VariableDeclaration>> varSequenceForBuf;
	// size_t idx = 0;
	// vector<size_t> slotSeq;
	// vector<size_t> offsetSeq;
	// size_t slot = 0;
	// size_t offset = 0;
	// for (auto var : stateVarDeclNodes) {
	// 	if (bits[idx] == 128)
	// 	{
	// 		varBit128.push_back(var);
	// 		// varSequenceForBuf.push_back(var);
	// 	}
	// 	else
	// 	{
	// 		subNodes.push_back(var);
	// 		varSequenceForBuf.push_back(var);
	// 		slotSeq.push_back(slot);
	// 		slot += 1;
	// 	}
	// 	idx += 1;
	// }
	// for (auto var128 : varBit128) {
	// 	subNodes.push_back(var128);
	// 	varSequenceForBuf.push_back(var128);

	// 	slotSeq.push_back(slot);
	// 	offsetSeq.push_back(offset);
	// 	offset += 16;
	// 	if(offset > 16) {
	// 		offset = 0;
	// 		slot += 1;
	// 	}
	// }
	// ASTPointer<ContractDefinition> contract = nodeFactory.createNode<ContractDefinition>(
	// 	name,
	// 	nameLocation,
	// 	documentation,
	// 	baseContracts,
	...
	// // 建立状态变量与buffer项的一一对应
	// idx = 0;
	// for (auto var : varSequenceForBuf) {
	// 	size_t nodeId = var->getNodeID();
	// 	// size_t varSlot = slotSeq[idx];
	// 	// size_t varOffset = offsetSeq[idx];
	// 	// value, uvalue, isSigned
	// 	// contract->m_stateVarBuffer[var] = make_pair{varSlot, varOffset};
	// 	contract->m_stateVarBuffer[nodeId] = make_pair(varSlot, varOffset);
	// 	idx++;
	// }
	// return contract;
...
// void addBuffItem(ContractDefinition contract, VariableDeclaration &var, size_t varSlot, size_t varOffset, int256 value, uint256 uvalue, bool isSigned) {
// 	contract.m_stateVarBuffer.push_back(StateVarBuffer{var, varSlot, varOffset, value, uvalue, isSigned});
// }
