...
void StorageOffsets::computeOffsets(TypePointers const& _types)
{
	...
	// pair (index, type): sort _types by type->storageBytes()
	std::vector<std::pair<size_t, Type const*>> sortedTypes;
	for(size_t i = 0; i < _types.size(); i++)
		sortedTypes.push_back(std::make_pair(i, _types[i]));
	// 使用标准库函数sort进行排序，lambda表达式定义比较函数
	// sort(_RandomAccessIterator __first, _RandomAccessIterator __last, _Compare __comp)；
	std::sort(sortedTypes.begin(), sortedTypes.end(), [](auto const& _a, auto const& _b){return _a.second->storageBytes() < _b.second->storageBytes();});
	// for (size_t i = 0; i < _types.size(); ++i)
	// 此时i是乱序，sortedTypes中的元素按type的size大小升序排列
	for (auto [i, type_i] : sortedTypes)
	{
...
void ContractType::computeStateVariables()
{
	...
	for (size_t index = 0; index < variables.size(); ++index)
		if (auto const* offset = offsets.offset(index))
			m_stateVariables.emplace_back(variables[index], offset->first, offset->second);
			// variablesAndOffsets.emplace_back(variables[index], offset->first, offset->second);
	// return variablesAndOffsets;
	// [var, buffer_ref, storage_ref, byte_offset]
	// vector<tuple<VariableDeclaration const*, u256, u256, unsigned>> m_bufferMaps;
	for (size_t index = 0; index < variables.size(); ++index)
		if (auto const* offset = offsets.offset(index))
			m_bufferMaps.emplace_back(variables[index], u256(index), offset->first, offset->second);
}

// vector<tuple<VariableDeclaration const*, u256, unsigned>> ContractType::stateVariables()
// {
	...
	m_declaration(&_varDecl)
{
	...
	// while中处理mapping和array,及相应的多重mapping和array
	while (true)
	{
