...
public:
	...
	size_t getNodeID() const { return m_id;}
protected:
	size_t const m_id = 0;
...
public:
	...
	// 	uint256 uvalue;
	// 	bool isSigned;
	// 	StateVarBuffer(VariableDeclaration &var,size_t varSlot. size_t varOffset, int256 value, uint256 uvalue, bool isSigned){
	// 		this.var = var;
	...
	// 	};
	// }
	// std::vector<StateVarBuffer> m_stateVarBuffer;

	// Simplyfy the buffer structure: 
	// just provide the storage position to index the BUFFER in evm
	// struct StateVarBuffer
	// {
	// 	size_t varSlot;
	// 	size_t varOffset;
	// }
	// pair<size_t, size_t> buffferPosition;
	// 用ID作为状态变量索引，而不用状态变量声明节点
	// std::map<VariableDeclaration, std::pair<size_t, size_t>> m_stateVarBuffer;
	// id->(slot, offset)
	// std::map<size_t, std::pair<size_t, size_t>> m_stateVarBuffer;
...
public:
	...
	enum Location { Unspecified, Storage, Memory, CallData, Buffer };
