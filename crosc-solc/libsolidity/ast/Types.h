...
enum class DataLocation { Storage, CallData, Memory, Buffer };
...
public:
	...
	{
		// 初始化ContractType对象时完成m_stateVariables和m_bufferMaps的计算
		computeStateVariables();
	}
	...
	std::vector<std::tuple<VariableDeclaration const*, u256, unsigned>> stateVariables() const {return m_stateVariables;}
	std::vector<std::tuple<VariableDeclaration const*, u256, unsigned>> m_stateVariables;
	std::vector<std::tuple<VariableDeclaration const*, u256, u256, unsigned>> bufferMaps() const { return m_bufferMaps;}
	std::vector<std::tuple<VariableDeclaration const*, u256, u256, unsigned>> m_bufferMaps;
