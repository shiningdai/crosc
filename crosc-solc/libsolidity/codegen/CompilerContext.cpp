...
// 类似addStateVariable()方法，建立起Buffer reference的一一映射
// 提示：addStateVariable()方法是在下面方法中被调用，参数_storageOffset, _byteOffset也是在其中获取并传进来的
// void ContractCompiler::registerStateVariables(ContractDefinition const& _contract)
// TODO:
void CompilerContext::addBufferVariable(
	VariableDeclaration const& _declaration,
	u256 const& _bufferIndex,
	u256 const& _storageOffset,
	unsigned _byteOffset
)
{
	m_bufferVariables[&_declaration] = make_tuple(_bufferIndex, _storageOffset, _byteOffset);
}
...
// buffer索引信息获取
tuple<u256, u256, unsigned> CompilerContext::bufferLocationOfVariable(Declaration const& _declaration) const
{
	auto it = m_bufferVariables.find(&_declaration);
	solAssert(it != m_bufferVariables.end(), "Variable not found in buffer.");
	return it->second;
}
