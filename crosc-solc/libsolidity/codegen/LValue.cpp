...
BufferItem::BufferItem(CompilerContext& _compilerContext, VariableDeclaration const& _declaration):
	LValue(_compilerContext, _declaration.annotation().type)
{
	solAssert(!_declaration.immutable(), "");
	// TODO: get location informtion of buffer item of the given variavle from m_context
	// auto const& location = m_context.storageLocationOfVariable(_declaration);
	auto const& location = m_context.bufferLocationOfVariable(_declaration);
	m_context << std::get<0>(location);
}

// 针对函数入口处、返回处、使用过程中的BufferItem的建立过程是不同的，要分别处理
// BufferItem::BufferItem(CompilerContext& _compilerContext, VariableDeclaration const& _declaration, bool init):
// 	LValue(_compilerContext, _declaration.annotation().type)
// {
// 	solAssert(!_declaration.immutable(), "");
// 	// TODO: get location informtion of buffer item of the given variavle from m_context
...
void BufferItem::retrieveValue(SourceLocation const&, bool _remove) const
{
	// stack: buffer_key
	if (!m_dataType->isValueType())
	{
	...
	// 从Buffer中加载不需要这过程了，只要一条BLOAD指令即可。问题是buffer的key如何生成和传递
	// TODO: load from buffer
	m_context << Instruction::BLOAD;
}

void BufferItem::storeValue(Type const& _sourceType, SourceLocation const& _location, bool _move) const
{
	CompilerUtils utils(m_context);
	solAssert(m_dataType, "");
	solAssert(_location.isValid(), "Invalid source location.");
	// stack: value storage_key
	if (m_dataType->isValueType())
	{
		...
		// _move = true;
		// if (!_move)
		// 	m_context << Instruction::DUP1 << Instruction::SWAP1;
		if (!_move)
			m_context << Instruction::DUP2 << Instruction::SWAP1;
		// [v, v, k] -> [v, k, v]
		// m_context << Instruction::SWAP1;
		// 是否usign 与 signed 间的转换？
		utils.convertType(_sourceType, *m_dataType, true);
		// [v, k, v] -> [v, v, k]
		// m_context << Instruction::SWAP1;
		// [v, v, k] -> [v]
		m_context << Instruction::BSTORE;
		// 移除
		// if (_move)
...
void BufferItem::setToZero(SourceLocation const&, bool _removeReference) const
{
	solAssert(m_dataType->isValueType(), "Clearing of unsupported type requested: " + m_dataType->toString());
	if (!_removeReference)
		CompilerUtils(m_context).copyToStackTop(sizeOnStack(), sizeOnStack());
	// [key] -> [key, 0] -> [0, key] -> []
	m_context << u256(0) << Instruction::SWAP1 << Instruction::BSTORE;
}
...
void StorageItem::retrieveValue(SourceLocation const&, bool _remove) const
{
	...
	if (!_remove)
		CompilerUtils(m_context).copyToStackTop(sizeOnStack(), sizeOnStack());
	// 如果是宽度为32字节的变量，直接从Buffer中加载出对应的值即可
	// POP 栈顶的 offset-0，BLOAD 从key处的位置加载值
	...
	else
	{
		...
		bool cleaned = false;
		// 这一通操作就是将Value从storage中加载后，右移offset个字节后的结果放到栈顶
		m_context
