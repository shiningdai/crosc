...
public:
	...
	// void compile(Expression const& _expression, bool _fromFunction = false);
	...
	// 是否只有mapping和arry类型的stateVar才会有accessor
	...
	void appendConstStateVariableAccessor(VariableDeclaration const& _varDecl);
		...
		// 一个合约函数中涉及到的状态变量引用计数
	std::map<VariableDeclaration const*, size_t>  m_varCounterE;
	// 给ExpressionCompiler增加一个成员变量bool m_fromFunction;
	// 从函数解析路径进入的给他置为true
	bool m_fromFunctionE = false;
