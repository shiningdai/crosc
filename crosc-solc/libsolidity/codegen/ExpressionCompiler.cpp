...
// void ExpressionCompiler::compile(Expression const& _expression, bool _fromFunction = false)
...
bool ExpressionCompiler::visit(Assignment const& _assignment)
{
	...
	// 由此进入Identifier的visit和赋值m_cum_currentLValue
	...
	else  // compound assignment
	{
		...
		// 变量引用（如Storage的slot,offset）栈用的栈空间个数
		unsigned lvalueSize = m_currentLValue->sizeOnStack();
		// 值占用的栈空间个数
		unsigned itemSize = _assignment.annotation().type->sizeOnStack();
		// 我不理解，这里在干嘛，执行两次copyToStackTop且交换参数后，栈里的内容不是没变吗？？
		if (lvalueSize > 0)
		{
...
// Buffer issue [1]: 这里缺乏处理，处理赋值表达式右值的时候到达了这里
// 传递给这里的参数没有告诉目标函数参数是否buffer有效setLValueFromDeclaration()
...
void ExpressionCompiler::setLValueFromDeclaration(Declaration const& _declaration, Expression const& _expression)
{
	...
	{
		if (m_fromFunctionE == true)
		{
			auto const& _var = dynamic_cast<VariableDeclaration const&>(_declaration);
			// 	Buffer issue [2]: 问题可能出在此处，始终没有进入if分支
			// solAssert(_vardebug, "");
			if (m_varCounterE[&_var] > 1)
				setLValue<BufferItem>(_expression, dynamic_cast<VariableDeclaration const&>(_declaration));
			else
