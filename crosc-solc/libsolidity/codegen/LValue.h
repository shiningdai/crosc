...
/**
 * @brief Reference to some item in Buffer.
 * Reference to some item in Buffer. On the stack this is <buffer key>
 */
class BufferItem: public LValue
{
public:
	/// Constructs the LValue and pushes the location of @a _declaration onto the stack.
	BufferItem(CompilerContext& _compilerContext, VariableDeclaration const& _declaration);
	unsigned sizeOnStack() const override { return 1; }
	void retrieveValue(langutil::SourceLocation const& _location, bool _remove = false) const override;
	void storeValue(
		Type const& _sourceType,
