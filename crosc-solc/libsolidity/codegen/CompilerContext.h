...
public:
	...
	void addBufferVariable(VariableDeclaration const& _declaration, u256 const& _bufferIndex, u256 const& _storageOffset, unsigned _byteOffset);
	...
	std::tuple<u256, u256, unsigned> bufferLocationOfVariable(Declaration const& _declaration) const;
...
private:
	...
	// CompilerContext类中有两个startFunction(xxx)
	// 1.在内部结构体类（FunctionCompilationQueue）中有一个，这个startFunction(_fun)的含义是：
	// 我们要开始编译_fun了，现在把他从m_functionsToCompile移除（如果有的话），并放到m_alreadyCompiledFunctions中标记吧
	// 2.CompilerContext类本身有一个startFunction(_fun)，
	// 这个startFunction函数重置函数特定成员，插入函数入口标签，并将函数标记为“具有代码-having code”。
	// 该注释通常用于编译器中将函数编译成可执行代码的过程中。它描述了函数编译的几个关键步骤：
	// 首先，该函数会重置一些与函数编译相关的成员，以确保编译过程中使用的数据是正确的。
	// m_functionCompilationQueue.startFunction(_function); （重置就是上面说的移除并加入到另一个成员的过程吧？）
	// 接着，编译器会为函数插入一个入口标签，以便在程序执行时可以通过该标签找到该函数的入口点。（this << functionEntryLabel(_function);）
	// 最后，编译器会将该函数标记为“具有代码”，表示该函数已经被编译成可执行代码，可以被程序调用执行。
	// 内部结构体类（FunctionCompilationQueue）及其成员变量和方法都是CompilerContext的私有成员，
	// 而CompilerContext类本身的startFunction(_fun)是public，是其他类的调用接口
	// 在ContractCompiler::visit(FunctionDefinition const& _function)中，首先就是m_context.startFunction(_function);
	...
	// 还需要一个m_stateVariables类似的成员存储变量的Buffer索引和偏移
	// TODO: implemet buffer reference map <var, bufferIndex, storageOffset, byteOffset>
	std::map<Declaration const*, std::tuple<u256, u256, unsigned>> m_bufferVariables;
