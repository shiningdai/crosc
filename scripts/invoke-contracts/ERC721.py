...
def invoke(contractAddress, CONTRACT_ABI, func_name):
    ...
    if func_name == "approve":
        return_val = m_contractInst.functions.approve(1).call()
    elif func_name == "allowance":
        return_val = m_contractInst.functions.allowance().call()
    elif func_name == "balanceOf":
        return_val = m_contractInst.functions.balanceOf(1).call()
    elif func_name == "mintToken":
        return_val = m_contractInst.functions.mintToken(1).call()
    elif func_name == "transferToken":
        return_val = m_contractInst.functions.transferToken(1, 200).call()
...
if __name__ == "__main__" :
    ...
    erc721 = ["approve", "allowance", "balanceOf", "mintToken", "transferToken"]
    ...
    time_sec = time.strftime("ERC721_base_%H%M%S.log", time.localtime())
    ...
    exe_times = OrderedDict((key, []) for key in erc721)
    dep_addrs = OrderedDict((key, []) for key in erc721)
    for r in range(5):
        CONTRACT_ABI = '../contracts/erc721/ERC721Extension.abi'
        CONTRACT_BIN = '../contracts/erc721/ERC721Extension.bin'
        for func in erc721:
            execution_times = []
            deployed_addrs = []
            for i in range(10):
